...
<script setup lang="ts">
    ...
    import { compareDocument } from '@/api/document'
    ...
    const getDocumentDiff = async () => {
        const { project_id, id } = unref(currentRoute).params
        const id1 = parseInt(id as any, 10)
        const selectedId = unref(changeDocSelectRef)
        const { data } = await fetchDiffApi({ project_id: project_id, id1, id2: selectedId })
        leftDoc.value = data.doc1 || {}
        rightDoc.value = data.doc2 || {}

        await useDocumentDetailInteractive('.diff-left-detail')
        await useDocumentDetailInteractive('.diff-right-detail')
    }
