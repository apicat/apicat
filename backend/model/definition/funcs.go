...
func GetDefinitionResponsesWithSpec(ctx context.Context, projectID string) (spec.HTTPResponseDefines, error) {
	var list []*DefinitionResponse
	...
	specResponses := make(spec.HTTPResponseDefines, 0)
	if len(list) > 0 {
		for _, r := range list {
...
func exportBuildDefinitionResponseTree(ctx context.Context, responses []*DefinitionResponse, parentID uint) spec.HTTPResponseDefines {
	result := make(spec.HTTPResponseDefines, 0)
	...
	for _, r := range responses {
		if r.ParentID == parentID {
			...
			specresponse := &spec.HTTPResponseDefine{
				ID:          int64(r.ID),
				Name:        r.Name,
				Type:        string(r.Type),
...
func GetDefinitionSchemasWithSpec(ctx context.Context, projectID string) (spec.Schemas, error) {
	var list []*DefinitionSchema
	...
	specSchemas := make(spec.Schemas, 0)
	if len(list) > 0 {
		for _, s := range list {
...
func exportBuildDefinitionSchemaTree(ctx context.Context, schemas []*DefinitionSchema, parentID uint) spec.Schemas {
	result := make(spec.Schemas, 0)
