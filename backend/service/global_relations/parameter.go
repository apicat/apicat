...
func ReadExceptParameterReference(ctx context.Context, content string) []uint {
	...
	for key, value := range request.Attrs.GlobalExcepts {
		if !arrutil.InArray[string](key, []string{string(global.ParameterInHeader), string(global.ParameterInPath), string(global.ParameterInHeader), string(global.ParameterInCookie)}) {
			continue
		}
		for _, v := range value {
...
func UnpackExceptGlobalParameter(ctx *gin.Context, gp *global.GlobalParameter) error {
	...
	for _, c := range collections {
		if !arrutil.InArray(c.ID, exceptCollectionIDs) {
