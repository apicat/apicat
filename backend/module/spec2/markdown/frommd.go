...
func ToDocment(md []byte) *spec2.Document {
	extensions := parser.CommonExtensions | parser.AutoHeadingIDs | parser.NoEmptyLineBeforeBlock
	p := parser.NewWithExtensions(extensions)
	doc := p.Parse(md)

	renderer := &markdownRenderDoc{
		tree: make([]*spec2.CollectionDoc, 0),
	}
	markdown.Render(doc, renderer)
	return &spec2.Document{Items: renderer.tree}
}

type markdownRenderDoc struct {
	tree  []*spec2.CollectionDoc
	state []*spec2.CollectionDoc
}
...
func (r *markdownRenderDoc) statePush(n *spec2.CollectionDoc) {
	last := r.stateNode()
	if last == nil {
		r.state = []*spec2.CollectionDoc{n}
...
func (r *markdownRenderDoc) stateNode() *spec2.CollectionDoc {
	n := len(r.state)
	if n == 0 {
		return nil
	}
