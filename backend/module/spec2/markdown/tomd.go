...
func ToMarkdown(root *spec2.Document) ([]byte, error) {
...
func (r *docRenderMarkdown) blockList(node *spec2.CollectionDoc) {
	r.listDepth++
...
func (r *docRenderMarkdown) content(prefix string, node *spec2.CollectionDoc) {
	r.startLine()
...
func (r *docRenderMarkdown) inline(s string, marks []*spec2.CollectionDoc) {
	if len(marks) == 0 {
		r.buf.WriteString(s)
		return
	}
...
func (r *docRenderMarkdown) renderMark(mark *spec2.CollectionDoc) (prefix, suffix string) {
	switch mark.Type {
	case "strong":
		return "**", "**"
	case "em":
...
func (r *docRenderMarkdown) renderNode(node *spec2.CollectionDoc) {
	switch node.Type {
	// inline
	case "text":
		s := node.Text
