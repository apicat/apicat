...
func (s *fromSwagger) parseParametersDefine(in *v2.Swagger) *spec.HTTPParameters {
	res := &spec.HTTPParameters{}
	s.parametersMapping = make(map[string]*spec.Parameter)
...
func (s *fromSwagger) parseGlobal(inp map[string]any) *spec.Global {
	res := &spec.Global{
	...
	for k, v := range global.(map[string]any) {
		...
		res.Parameters.Add(k[:in], p)
		s.globalParamtersMappint[p.Name] = struct{}{}
	}
	return res
}
	...
	for key, res := range in.Responses.Definitions {
		...
		if res.Headers != nil {
			for k, v := range res.Headers {
				header = append(header, &spec.Parameter{
					Name: k,
					Schema: &jsonschema.Schema{
						Type:        jsonschema.CreateSliceOrOne(v.Type),
						Format:      v.Format,
...
func (s *fromSwagger) parseCollections(in *v2.Swagger, paths *v2.Paths) []*spec.Collection {
...
func (s *toSwagger) parseResponse(in *spec.Spec, res *spec.HTTPResponseDefine) map[string]any {
	if res.Reference != nil {
		...
		if strings.HasPrefix(ref, "#/definitions/responses/") {
			x := in.Definitions.Responses.LookupByID(
