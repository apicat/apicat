...
export function getChildState(node: Node[]): TreeNodeChildState {
  let all = true
  let none = true
...
const getPropertyFromData = function (node: Node, prop: string): any {
  const props = node.store.props as any
...
class Node {
  ...
  initialize() {
    ...
    if (!Array.isArray(this.data))
      markNodeData(this, this.data as any)
    ...
    if (typeof props.draggable === 'function')
      this.draggable = computed(props.draggable(this.data))
  }
  ...
  insertChild(child?: FakeNode | Node, index?: number, batch?: boolean): void {
    ...
    else this.childNodes.splice(index, 0, child as Node)
  ...
  getChildren(forceInit = false): any {
    // this is data
    if (this.level === 0)
      return this.data
  ...
  updateChildren(): void {
    ...
    const newDataMap = {} as any
    const newNodes = [] as any

    newData.forEach((item, index) => {
      const key = item[NODE_KEY]
      const isNodeExists
        = !!key && oldData.findIndex((data: any) => data[NODE_KEY] === key) >= 0
      if (isNodeExists)
        newDataMap[key] = { index, data: item }
      else newNodes.push({ index, data: item })
    })
    ...
    newNodes.forEach(({ index, data }: any) => {
