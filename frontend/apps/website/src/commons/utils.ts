...
export function checkDepth<T>(node: T, subKey: keyof T, isLeaf: (node: T) => boolean) {
  function getin(node: T, depth: number): number {
    ...
    const des: number[] = []
    if (!li || li.length <= 0)
      des.push(isLeaf(node) ? depth - 1 : depth)
    for (let i = 0; i < li.length; i++)
...
export function flattenObjectDot(obj: any, prefix = '') {
  return Object.keys(obj).reduce((acc: any, key) => {
    ...
    if (typeof obj[key] === 'object')
      acc = acc.concat(flattenObjectDot(obj[key], pre + key))
  ...
  return [
    {
      validator(_: any, v: any, c: any) {
        if (!v || !isURL(v, { allow_localhost, require_protocol }))
          return c(new Error(msg))
        return c()
      },
      trigger: 'blur',
    },
